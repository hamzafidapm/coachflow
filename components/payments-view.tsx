...
import { barsFor } from '@/lib/data/payments';
...
export function PaymentsView({
  ...
  grossVolumeMonthly: string;
  grossVolumeYearly: string;
  ...
  const grossVolume = billing === 'Monthly' ? grossVolumeMonthly : grossVolumeYearly;
        ...
        <div className="p-[22px] bg-surface border border-border rounded-card">
          <div className="flex items-start justify-between gap-4">
            <div>
              ...
              <div className="mt-2 text-[32px] font-bold tracking-tight">{grossVolume}</div>
              <div className="mt-2 flex items-center gap-1.5 text-[12.5px] text-text-dim">
                {billing === 'Monthly' ? 'This calendar month' : 'This calendar year'} · succeeded payments
              </div>
            </div>
