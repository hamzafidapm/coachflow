...
export function SettingsTabs({ active, onChange }: { active: SettingsTab; onChange: (tab: SettingsTab) => void }) {
  return (
    <div className="p-2 bg-surface border border-border rounded-card flex flex-col gap-0.5">
      {settingsTabDefs.map((t) => {
        ...
        return (
          <button
            ...
            className="flex items-center gap-2.5 h-10 px-3 rounded-[10px] text-[13px] font-semibold text-left transition-colors hover:bg-[#16191D] hover:text-text"
